defmodule EtherCAT.Drivers.Generic2 do
  @moduledoc """
  Generic EtherCAT slave driver (new architecture).
  ...
  alias EtherCAT.{Master2, Nif, Slave.Driver}
  ...
  def handle_call({:read, pdo_name, entry_name}, _from, state) do
    ...
    case Master2.read_pdo_entry(state.master, domain, unique_name) do
      {:ok, binary} ->
  ...
  def handle_call({:write, pdo_name, entry_name, value}, _from, state) do
    ...
    case Driver.encode_pdo_value(type, value) do
      {:ok, binary} ->
        result = Master2.write_pdo_entry(state.master, domain, unique_name, binary)
        {:reply, result, state}
  ...
  defp get_sync_manager(state, sync_index) do
    Master2.get_sync_manager(state.master, state.position, sync_index)
  end

  defp get_pdo(state, sync_index, pdo_pos) do
    Master2.get_pdo(state.master, state.position, sync_index, pdo_pos)
  end

  defp get_pdo_entry(state, sync_index, pdo_pos, entry_pos) do
    Master2.get_pdo_entry(state.master, state.position, sync_index, pdo_pos, entry_pos)
  end
