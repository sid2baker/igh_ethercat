defmodule IODiagnosisTest do
  ...
  def run do
    ...
    Logger.info("Input pdo_6000:1: offset=#{in_offset} bits (byte #{div(in_offset, 8)}), size=#{in_size}")
    Logger.info("Output pdo_7000:1: offset=#{out_offset} bits (byte #{div(out_offset, 8)}), size=#{out_size}")
